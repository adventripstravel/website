...
function navScrollDown(nv_target, nv_class, nv_height, lt_target, lt_url_down)
{
    if (lt_target)
        var lt_url_up = '../' + $(nv_target).find(lt_target).attr('src');
    ...
    var nav = {
        ...
        scroller: function()
        {
            if ($(document).scrollTop() > nv_height)
            {
                $(nv_target).addClass(nv_class);

                if (lt_target)
                    $(nv_target).find(lt_target).attr('src', lt_url_down);
            }
            else
            {
                $(nv_target).removeClass(nv_class);

                if (lt_target)
                    $(nv_target).find(lt_target).attr('src', lt_url_up);
            }
        }
    }
